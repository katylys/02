...
export const Redirect = async (req: Request, resp: Response, db: Db) => {
    ...
    const existURL = await db.collection<DB.URLs>(DB.storeURLs).findOne({
    ...
    const updateUsage = await db.collection<DB.URLs>(DB.storeURLs).findOneAndUpdate({
        shortURL: req.params.shortURL,
    },
        {
            $inc: {
    ...
    return resp.redirect(existURL.longURL)
}
